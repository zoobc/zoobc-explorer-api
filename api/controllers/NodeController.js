      ...
      RedisCache.get(cacheNodes, (errRedis, resRedis) => {
        ...
        if (resRedis) {
          this.sendSuccessResponse(
            res,
            responseBuilder.setData(resRedis.data).setPaginate(resRedis.setPaginate).setMessage('Nodes fetched successfully').build()
          );
          return;
        }
        ...
        this.service.paginate({ page, limit, fields, where, order }, (err, result) => {
          ...
          RedisCache.set(cacheNodes, result.data, err => {
            ...
            this.sendSuccessResponse(
              res,
              responseBuilder.setData(result.data).setPaginate(result.paginate).setMessage('Nodes fetched successfully').build()
            );
            return;
          });
        });
  ...
  async getOne(req, res) {
    ...
    try {
      ...
      RedisCache.get(cacheNode, (errRedis, resRedis) => {
        ...
        if (resRedis) {
          this.sendSuccessResponse(res, responseBuilder.setData(resRedis).setMessage('Node fetched successfully').build());
          return;
        }
        ...
        this.service.findOne({ NodeID: nodeID }, (err, result) => {
          ...
          RedisCache.set(cacheNode, result, err => {
            ...
            this.sendSuccessResponse(res, responseBuilder.setData(result).setMessage('Node fetched successfully').build());
            return;
          });
        });
      });
